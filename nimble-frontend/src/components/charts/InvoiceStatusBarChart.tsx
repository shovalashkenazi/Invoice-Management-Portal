...
const InvoiceStatusBarChart = ({ data, currency }: { data: StatusData[]; currency: string }) => {
  if (!data?.length)
    return (
  ...
  return (
    <Box mt={9} w="100%" h="350px">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 40, right: 30, left: 80, bottom: 50 }}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="status" tick={{ fontSize: 12 }} height={50} />
          <YAxis tickFormatter={(value) => value.toLocaleString(undefined, { style: 'currency', currency })} tick={{ fontSize: 12 }} width={70} />
